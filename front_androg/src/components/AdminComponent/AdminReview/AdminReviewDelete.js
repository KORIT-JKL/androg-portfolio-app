...
const AdminReviewDelete = () => {
  ...
  const getreviews = useQuery(
    ...
    async () => {
      ...
      const response = await axios.get("http://52.79.158.206/admin/reviews", option);
      return response;
    },
    {
      onSuccess: (response) => {
  ...
  const reviewDelete = async (e) => {
    ...
    if (window.confirm("삭제하시겠습니까?")) {
      const response = await axios.delete("http://52.79.158.206/admin/reviews/delete", option);
      setRefresh(true);
      alert("삭제 완료");
      getreviews.refetch();
      return response;
