...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if(userEntity == null) {
			...
			response
			.sendRedirect(
					"http://sangsanggooo.s3-website.ap-northeast-2.amazonaws.com/auth/oauth2/register"
			...
			if(StringUtils.hasText(userEntity.getProvider())) {
				...
				if(!userEntity.getProvider().contains(provider)) {
					response.sendRedirect("http://sangsanggooo.s3-website.ap-northeast-2.amazonaws.com/auth/oauth2/merge"	
							+ "?provider="
							+ provider
							+ "&email=" + email);
					return;
				}
				response.sendRedirect("http://sangsanggooo.s3-website.ap-northeast-2.amazonaws.com/auth/oauth2/login"	
